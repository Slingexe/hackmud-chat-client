...
function Formatter(message, localuser, sanitize) {
    ...
    if (!sanitize) {
        ...
        let formattedUser = `[0;33m${message.from_user}[0;0m`;
        let formattedResipiant
        if (message.to_user) {formattedResipiant = `[0;33m${message.to_user}[0;0m`;}
        const formattedChnlBlue = `[0;34m${message.channel}[0;0m`;
        const formattedChnlPink = `[0;35m${message.channel}[0;0m`;
        const formattedTellT = `[0;34mto[0;0m`;
        const formattedTellF = `[0;34mfrom[0;0m`;
        ...
        if (message.is_join) { // Checks if the message is a join message
            formattedMessage = `\`\`\`ansi\n${formattedTime} ${formattedChnlBlue} ${formattedUser} ${messageBord} ${convertedMessage} ${messageBord}\n\`\`\``;
        } else { // If its not a join message, it's a regular message
            if (!message.channel) { // Then we check to see if the message is a tell of some kind
                if (message.from_user == localuser) { // We check to see if the tell message was sent by us.
                    // This message was sent by us
                    formattedMessage = `\`\`\`ansi\n${formattedTime} ${formattedTellT} ${formattedResipiant} ${messageBord} ${convertedMessage} ${messageBord}\n\`\`\``;
                } else {
                    // This message was sent to us
                    formattedMessage = `\`\`\`ansi\n${formattedTime} ${formattedTellF} ${formattedUser} ${messageBord} ${convertedMessage} ${messageBord}\n\`\`\``;
                }
            } else { // If it's not a tell, it's a regular message
